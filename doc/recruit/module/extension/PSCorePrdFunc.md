...
##### 全部数据（读写） :id=PSCorePrdFunc-ALL_RW
...
| --------| --------| -------- |------------|------------|------------|
| 打开卡片详情页面 | open_info_view | 打开卡片详情页面 |单项数据|用户自定义||
| 跳转插件设置页面 | skip_setting | 跳转插件设置页面 |无数据|用户自定义||
...
<el-anchor-link :href="`#/module/extension/PSCorePrdFunc?id=数据权限`">
  数据权限
</el-anchor-link>
